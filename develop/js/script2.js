...
let forcastWeather = JSON.parse(localStorage.getItem("forcastWeatherData")); // null
// console.log(currentWeather);
...
let getLocationCurrentWeather = function (locationSearched) {
    ...
    fetch (apiURL)
        .then (function (response){
            ...
            let fullAPIURL = "https://api.openweathermap.org/data/2.5/weather?lat=" + currentLat + "&lon=" + currentLon + "&appid=" + apiKey + "&units=metric";
            ...
            fetch (fullAPIURL)
                .then (function (currentresponse){
                    ...
                    alert("fullAPIURL fetch Error: " + response.statusText);
                    }
           });
        })
        .catch (function (error) {
            alert("getLocationCurrentWeather Error: Unable to connect to WeatherService");
        });
...
// let getLocationForcast = function (locationSearched){
//     let forcastApiURL = apiBaseURL +locationSearched + "&limit=1" + apiKey + "&units=metric";
//     fetch (forcastApiURL)
//         .then (function (response){
//             response.json().then(function (data) {
//                 console.log(data);
...
//                 console.log(data);
//             if (response.ok) {
//                 localStorage.setItem("forcastWeatherData", JSON.stringify(data));
//                 forcastWeather = JSON.parse(localStorage.getItem("forcastWeatherData")); 
//                 console.log(forcastWeather + " (source: getLocationForcast)"); //used for debugging
//                 // displayForcastWeather(); 
//             }else{
//             alert("Forcast Error: " + response.statusText);
//             }
//            });
//         })
//         .catch (function (error) {
